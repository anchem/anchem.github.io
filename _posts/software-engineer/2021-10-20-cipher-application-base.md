...
Hash算法有着广泛的用途，包括但不限于：

- 消息摘要。数字摘要的大小一般远远小于摘要的消息大小，通过这种不可逆的压缩数据，可以大大减小为保证数据完整性而传输的额外信息。
- 数字指纹。对于数字信息的微小改动，都会造成信息的Hash值的巨大变化，这种敏感的特性非常适合用于保护数据完整性、防篡改。
- 单向散列。攻击者无法从Hash值还原出原始消息，可以用于保存口令等。推荐使用PBKDF2算法，并加盐防御彩虹表攻击。

### MAC算法

MAC，Message Authentication Code，消息认证码。使用一个密钥作用于消息，生成一个固定大小的数据块，加入到消息中，成为MAC，用于对消息的完整性进行认证，防止消息被篡改。

其与Hash算法的不同在于，MAC需要密钥参与运算。推荐使用HMAC。

**【HMAC】**

HMAC利用一个哈希函数和一个密钥来计算消息的MAC值，用于校验消息的完整性和身份认证。

- 密钥的长度至少与哈希函数输出的摘要值的长度一样；
- 哈希函数应选择安全哈希函数；
- 禁止用Hash(key||message)拼接的方式进行哈希运算。
...
密码协议（也称为加密协议）使用密码学的方法保证通信双方数据传输的机密性和完整性。

常见的密码协议有SSL/TLS，SSH等。

### SSL/TLS

该类协议常用与通信双方的身份认证，会话密钥协商，通信内容加密和完整性保护。

当前禁止使用的版本为：`SSL2.0`、`SSL3.0`、`TLS1.0`

推荐使用的版本为：**TLS1.2**，**TLS1.3**版本。

### SSH

SSH用于为远程登录会话和其他网络服务提供安全性。

安全使用规则：

- SSH协议中禁止使用CBC模式；
- 禁止在密码协议中使用截短的消息认证码；
- 优先选择ECC密钥交换算法和数字签名算法，优先选择HMAC-SHA2消息认证算法。

## 实践原则

### 不要使用私有、非标准的密码算法

真正的密码系统应该做到就算被所有人知道系统的运作过程，仍然是安全的。

密码系统的安全性应该取决于密钥的保密性而不是算法的保密。如果安全受到破坏，只需要替换密钥，而不必替换整个系统。

### 设计阶段应当考虑算法的敏捷性

算法模块应当独立，代码接口有较好的扩展性，易于替换升级。
